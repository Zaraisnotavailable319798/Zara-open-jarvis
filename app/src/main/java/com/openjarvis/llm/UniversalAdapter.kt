...
class UniversalAdapter(private val context: Context) {
    ...
    fun getActiveProvider(): LLMProvider {
        ...
        return when (name) {
            ...
            "Local (LlamaCpp)" -> {
                ...
                val tier =
                    mm.getDownloadedTier()
                        ?: ModelManager.ModelTier.MINIMUM

                LlamaProvider(
                    mm.getModelPath(tier),
                    tier.displayName
                )
            }
    ...
    suspend fun testConnection(): Result<Long> {
        return getActiveProvider().testConnection()
    }
    ...
    companion object {
        ...
        fun isLocalModelLoaded(): Boolean {
            ...
            return state::class.simpleName == "ModelLoaded" ||
                state::class.simpleName == "Loaded"
        }

        suspend fun loadLocalModel(
            tier: ModelManager.ModelTier
        ): Result<Unit> {
            val manager = modelManager
                ?: return Result.failure(
                    Exception("ModelManager not initialized")
                )

            return manager.loadModel(tier)
        }
