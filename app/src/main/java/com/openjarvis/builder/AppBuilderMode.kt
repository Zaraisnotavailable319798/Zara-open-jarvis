...
class AppBuilderMode(private val context: Context) {
    ...
    private val outputDir =
        File(context.filesDir, "built_apps").also { it.mkdirs() }

    private val llm = UniversalAdapter(context)

    private val _state =
        MutableStateFlow<BuildState>(BuildState.Idle)
    ...
    suspend fun analyzeRequirements(
        prompt: String
    ): AppSpec = withContext(Dispatchers.IO) {

        _state.value =
            BuildState.Planning("Analyzing requirements...")
        ...
        val result =
            llm.complete(systemPrompt, prompt)

        val json =
            result.getOrNull()
                ?: throw Exception("Failed to analyze requirements")
    ...
    private fun buildFileList(
        ...
        return listOf(
            ...
            FileSpec(
                "app/src/main/java/com/jarvisbuilt/${spec.packageSafeName}/MainActivity.kt",
                FileType.KOTLIN
            ),
            FileSpec(
                "app/src/main/java/com/jarvisbuilt/${spec.packageSafeName}/ui/theme/Theme.kt",
                FileType.KOTLIN
            ),
            FileSpec(
                "app/src/main/java/com/jarvisbuilt/${spec.packageSafeName}/data/AppDatabase.kt",
                FileType.KOTLIN
            ),
            FileSpec(
    ...
    private suspend fun generateFileContent(
        ...
        val prompt =
            buildString {
                ...
                appendLine(
                    "You are generating file: ${fileSpec.path}"
                )

                appendLine(
                    "For app: ${spec.appName} — ${spec.description}"
                )
                ...
                appendLine(
                    "1. Kotlin + Jetpack Compose only — no XML layouts"
                )

                appendLine(
                    "2. Material3 components only"
                )
                ...
                appendLine(
                    "4. Room for all data persistence"
                )

                appendLine(
                    "5. ViewModel + StateFlow for state"
                )

                appendLine(
                    "6. No hardcoded strings — strings.xml"
                )
                ...
                appendLine(
                    "8. minSdk 26, targetSdk 34"
                )

                appendLine(
                    "9. All coroutines in viewModelScope"
                )

                appendLine(
                    "Generate the complete, working file content."
                )
            }
    ...
    private fun generateStubFile(
        ...
        return when (fileSpec.type) {
            ...
            FileType.MANIFEST ->
                """<manifest package="com.jarvisbuilt.${spec.packageSafeName}"/>"""

            FileType.KOTLIN ->
                """
                package com.jarvisbuilt.${spec.packageSafeName}
