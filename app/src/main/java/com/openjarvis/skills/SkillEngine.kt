...
class SkillEngine(private val context: Context) {
    ...
    suspend fun loadSkills() = withContext(Dispatchers.IO) {
        ...
        val jsonFiles = skillsDir.listFiles { file ->
    ...
    private fun parseSkill(json: JSONObject): Skill {
        val triggerPhrases = json.optJSONArray("triggerPhrases")?.let { arr ->
            (0 until arr.length()).map { index ->
                arr.getString(index)
            }
        } ?: emptyList()

        val variables = json.optJSONArray("variables")?.let { arr ->
        ...
        val actionTemplate = json.optJSONArray("actionTemplate")?.let { arr ->
            (0 until arr.length()).map { i ->
                val obj = arr.getJSONObject(i)
    ...
    fun buildContext(
        ...
        val actionsJson = skill.actionTemplate.joinToString(", ") { action ->
            val value = action.value?.let { actionValue ->
                vars.entries.find { entry ->
                    entry.key in actionValue
                }?.value ?: actionValue
            } ?: ""
...
Actions: [$actionsJson]
    ...
    private fun saveSkill(skill: Skill) {
        val json = JSONObject().apply {
            ...
            put(
                "triggerPhrases",
                JSONArray(skill.triggerPhrases)
            )
                ...
                JSONArray(
                    skill.variables.map {
                        JSONObject()
                            ...
                            .put("extractFrom", it.extractFrom)
                    }
                )
            )
