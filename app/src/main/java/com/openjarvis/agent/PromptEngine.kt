...
class PromptEngine(private val context: Context) {
    ...
    suspend fun analyzeIntent(prompt: String): Intent =
        withContext(Dispatchers.IO) {

            val resolvedPrompt =
                conversationContext.resolveReferences(prompt)
    ...
    private fun parseIntentFromJson(
        rawPrompt: String,
        json: JSONObject
    ): Intent {

        val phases =
            json.optJSONArray("phases")?.let { arr ->
        ...
        val conditions =
            json.optJSONArray("conditions")?.let { arr ->
                (0 until arr.length()).map { i ->
                    parseCondition(
                        arr.getJSONObject(i)
                    )
        ...
        val appsRequired =
            json.optJSONArray("appsRequired")?.let { arr ->
                (0 until arr.length()).map { i ->
                    arr.getString(i)
                }
            } ?: emptyList()
        ...
        val inputsObj =
            json.optJSONObject("dataInputs")

        inputsObj?.keys()?.forEach { key ->
            dataInputs[key] =
                inputsObj.optString(key, "")
        }
    ...
    private fun parsePhase(
        ...
        val failureStrategy =
            when (
                ...
                "retry" ->
                    PhaseFailureStrategy.Retry

                "abort" ->
                    PhaseFailureStrategy.Abort

                "ask_user" ->
                    PhaseFailureStrategy.AskUser(
                        json.optString(
                            "question",
