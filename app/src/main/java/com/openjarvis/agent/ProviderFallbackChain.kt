...
class ProviderFallbackChain(private val context: Context) {
    ...
    private val fallbackOrder: List<String>
        get() = prefs.getString(
            "fallback_order",
            "Groq,Google Gemini,OpenRouter,Anthropic Claude,OpenAI"
    ...
    suspend fun completeWithFallback(
        ...
        for ((provider, name) in providers) {
            ...
            // LLMProvider.complete() already returns Result<String>,
            // so don't wrap it inside runCatching.
            ...
            val errorMsg = result.exceptionOrNull()?.message ?: "Unknown provider error"

            if (errorMsg.contains("429", ignoreCase = true)) {
                cooldownMap[name] = System.currentTimeMillis() + 60_000

                _notification.value =
                    "Rate limited by $name, trying next provider..."
            }
        ...
        return Result.failure(
            Exception("All providers failed or rate limited")
        )
    }
    ...
    fun translate(
        ...
        return when {
            ...
            error.message?.contains("NullPointerException") == true ->
                ...
                "Something went wrong: ${error.message?.take(100) ?: "Unknown error"}"
        }
    }
}
